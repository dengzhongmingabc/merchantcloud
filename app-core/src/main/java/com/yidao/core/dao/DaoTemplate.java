...
public class DaoTemplate {
	protected static final Log log = LogFactory.getLog(DaoTemplate.class);
	//注入实体管理器
	@PersistenceContext
	...
	public DaoTemplate(Class<EntityObj> clazz) {
		this.clazz = clazz;
	}
	...
	public EntityObj queryById(Object id) {
		// TODO Auto-generated method stub
	...
	public  void remove(Long id)throws Exception {
		EntityObj entity = this.queryById(id);
		if (entity != null)
	 ...
	 */
	public void remove(EntityObj entity)throws Exception {
		if (entity == null){
			return;
		} else {
			entityManager.remove(entity);
	...
	public <T> void remove(Class<T> clazz,Long id)throws Exception {
		T entity = this.queryById(clazz, id);
		if (entity != null)
			entityManager.remove(entity);
	}
	/**
	 ...
	 */
	public int querySQLTotalCnt(final String sql, final Map<String,String> args)throws Exception {
		if (log.isDebugEnabled()) {
	...
	public int querySQLTotalCnt(final String sql)throws Exception {
		return querySQLTotalCnt(sql, null);
	}
	
	/**
	 * jpql分布查询
	 */
	public int queryJPQLTotalCnt(final String jpql,final Map<String,String> args)throws Exception {
	 ...
	 */
	public List queryByJPQL(final String jpql, final Map<String,String> args)throws Exception {
		if (log.isDebugEnabled()) {
			log.debug((new StringBuilder("queryNum :")).append(jpql).toString());
		}
	 ...
	 * 执行sql查询语句
	 */
	public List<EntityObj> queryByJPQL(final String jpql, final Map<String,String> args, final int start, final int size)throws Exception {
		if (log.isDebugEnabled()) {
			log.debug((new StringBuilder("queryNum :")).append(jpql).toString());
		}
		javax.persistence.Query query = entityManager.createQuery(jpql).setFlushMode(FlushModeType.COMMIT);
	 ...
	 * 原生sql分页查询
	 */
	public List<EntityObj> queryBySQL(final String sql, final Map<String,String> args, Class clazz, final int start, final int size)throws Exception{
		if (log.isDebugEnabled()) {
			log.debug((new StringBuilder("queryNum :")).append(sql).toString());
		}
		javax.persistence.Query query = entityManager.createNativeQuery(sql,clazz).setFlushMode(FlushModeType.COMMIT);
	 ...
	 */
	public List queryMapBySQL(final String sql)throws Exception {
		if (log.isDebugEnabled()) {
			log.debug((new StringBuilder("queryNum :")).append(sql).toString());
		}
	 ...
	 */
	public List queryMapBySQL(final String sql,final Map<String,String> args,final int start,final int size)throws Exception {
		if (log.isDebugEnabled()) {
			log.debug((new StringBuilder("queryNum :")).append(sql).toString());
		}
		javax.persistence.Query query = entityManager.createNativeQuery(sql).setFlushMode(FlushModeType.COMMIT);
